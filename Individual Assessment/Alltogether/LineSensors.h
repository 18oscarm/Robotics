...
class LineSensors_c {
  ...
  public:
    ...
    float minimum[NUM_SENSORS] = {0}; // All elements initialized to 0
    float maximum[NUM_SENSORS] = {1023, 1023, 1023, 1023}; // Manually set values
    ...
    // to perform and read of the line sensors using
    // the ADC.
    ...
    void calibrate_line_sensors(){//unsigned long calibration_length){
...
//      unsigned long end_time = start_time + calibration_length;
    ...
    bool online(){
      calcCalibratedADC();
      for ( int sensor = 0; sensor < NUM_SENSORS; sensor++ ){
        if (calibrated[sensor] > LINE_THRESHOLD){
          return true;
        }
      }
      return false;
