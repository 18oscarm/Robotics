...
short demand_storage_encoding(float wheel_demand) { // we need to store values between 1 and 0.0001
  wheel_demand = wheel_demand*10000;
  short trunc_demand = (short)wheel_demand;
  return trunc_demand;
...
float revert_demand_to_float(short scaled_demand) {
  float float_scaled_demand = (float)scaled_demand;
  float new_demand = float_scaled_demand/10000;
  return new_demand;
}
//============================
//ODOMETRY
  ...
  if (total_exp_time > EXPERIMENT_END_MS) {
    ...
    if( elapsed_time > results_interval_ms ) { //saves all data
      ...
      if(results_index < MAX_RESULTS) {
            results[ results_index ][0] = demand_storage_encoding(left_wheel_demand); // save demand to left wheel
            results[ results_index ][1] = demand_storage_encoding(right_wheel_demand); // save demand to right wheel
            results[ results_index ][2] = angle_storage_encoding(pose.theta); // save theta (from kinematics?)
            results[ results_index ][3] = bumper_storage_encoding(bumpers.left_last_bumper); // save left? bumper
            results[ results_index ][4] = bumper_storage_encoding(bumpers.right_last_bumper); // save right? bumper
            results_index++;
...
void experiment_finished() {
  ...
  int result;
  Serial.println("Sample, left wheel demand, right wheel demand, Theta, LeftB, RightB\n");
  for (result = 0; result < MAX_RESULTS; result++) {
  Serial.print( result );
        Serial.print(",");
        Serial.print( revert_demand_to_float(results[ result ][0] )); // X
        Serial.print(",");
        Serial.print( revert_demand_to_float(results[ result ][1] )); // Y
        Serial.print(",");
