...
void running_experiment() {
  ...
  //Serial.println(EXPERIMENT_END_MS - total_exp_time );
  ...
  if (total_exp_time > EXPERIMENT_END_MS) {
    ...
    if( elapsed_time > results_interval_ms ) { //saves all data
      ...
      if(results_index < MAX_RESULTS) {
            ...
            results[ results_index ][0] = angle_storage_encoding(pose.theta); // save theta (from kinematics?)
            results[ results_index ][1] = bumper_storage_encoding(bumpers.calibrated_digital[0]); // save left? bumper
            results[ results_index ][2] = bumper_storage_encoding(bumpers.calibrated_digital[1]); // save right? bumper
            results_index++;
...
void experiment_finished() {
  ...
  Serial.println("Sample, Theta, LeftB, RightB\n");
  
  for (result = 0; result < MAX_RESULTS; result++) {
  Serial.print( result );
        Serial.print(",");
        Serial.print( radian_to_degree(revert_angle_to_float(results[ result ][0] ))); // theta
        Serial.print(",");
        Serial.print( revert_bump_to_float(results[ result ][1] )); // LB
        Serial.print(",");
        Serial.print( revert_bump_to_float(results[ result ][2]) ); // RB
